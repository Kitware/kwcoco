...
def main(**kw):
    ...
    for gid, img in ub.ProgIter(coco_dset.imgs.items(), total=coco_dset.n_images):
        ...
        for aid in aids:
            ...
            tl_x, tl_y, br_x, br_y = (
                kwimage.Boxes([ann['bbox']], 'xywh').toformat('tlbr').data[0].tolist()
            )
            ...
            for kp in ann.get('keypoints', []):
                if 'keypoint_category_id' in kp:
                    cname = coco_dset._resolve_to_kpcat(kp['keypoint_category_id'])[
                        'name'
                    ]
                ...
                kp_x, kp_y = kp['xy']
                row.append(
                    '(kp) {} {} {}'.format(cname, round(kp_x, 3), round(kp_y, 3))
                )
            ...
            for note_key in note_fields:
                if note_key in ann:
                    row.append(
                        '(note) {}: {}'.format(
                            note_key, repr(ann[note_key]).replace(',', '<comma>')
                        )
                    )
