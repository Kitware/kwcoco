...
tp, tn, fp, fn, B = sympy.symbols(
    ['tp', 'tn', 'fp', 'fn', 'B'], integer=True, negative=False
)
numer = tp * tn - fp * fn
...
values = {
    fn: np.random.rand() * 10,
    fp: np.random.rand() * 10,
    tp: np.random.rand() * 10,
    tn: np.random.rand() * 10,
}
print(geometric_mean.subs(values))
print(abs(mcc).subs(values))

delta = sympy.simplify(
    sympy.simplify(geometric_mean) - sympy.simplify(sympy.functions.Abs(mcc))
)
