...
class CocoSqlIndex(object):
    ...
    def build(index, parent):
        ...
        index.vidid_to_gids = SqlIdGroupDictProxy(
            session, Image.id, Image.video_id)


from kwcoco.coco_dataset import MixinCocoJSONAccessors, MixinCocoAccessors  # NOQA
...
class CocoSqlDatabase(MixinCocoJSONAccessors, MixinCocoAccessors):
    ...
    def __init__(self, uri=None, img_root=None):
        if uri is None:
            uri = self.MEMORY_URI
        self.uri = uri
