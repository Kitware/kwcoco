...
def benchmark_large_hyperspectral():
    ...
    for kw in ub.named_product(basis):
        ...
        spectral.envi.save_image(
            str(spy_fpath), imdata, interleave='BSQ', ext='bsq', force=True
        )
        fpath = spy_fpath
        ...
        if 1:
            ...
            for slices in gdal_prog:
                ...
                data = rio_ds.read(
                    indexes=indexes,
                    window=((sl_y.start, sl_y.stop), (sl_x.start, sl_x.stop)),
                )
        ...
        if 1:
            ...
            for slices in spy_prog:
                ...
                sl_y, sl_x = slices
                final_data = spy_img.read_subregion(
                    (sl_y.start, sl_y.stop), (sl_x.start, sl_x.stop)
                )
    ...
    results['megabytes'] = (
        results['nbands'] * np.prod(slider.window) * imdata.dtype.itemsize / MB
    )
    print(results)
